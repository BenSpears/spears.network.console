...
(function () {
  ...
  function strength(bits) {
    if (bits < 45) return { label: "fair", color: "#f2c14e" };
    if (bits < 70) return { label: "strong", color: "var(--accent-2)" };
    return { label: "very strong", color: "var(--accent)" };
  }

  document.addEventListener("DOMContentLoaded", function () {
    var $ = function (id) { return document.getElementById(id); };
    var words = $("dw-words"), wordsVal = $("dw-wordsval"), sep = $("dw-sep"), go = $("dw-go"),
        out = $("dw-out"), val = $("dw-value"), bar = $("dw-bar"), bitsEl = $("dw-bits"),
        rating = $("dw-rating"), copy = $("dw-copy");
    if (!go) return;
    var perWord = Math.log2(WORDS.length);
    function gen() {
      var n = parseInt(words.value, 10), parts = [];
      for (var i = 0; i < n; i++) parts.push(WORDS[randInt(WORDS.length)]);
      val.textContent = parts.join(sep.value);
      var bits = Math.round(n * perWord);
      bitsEl.textContent = bits + " bits entropy (" + n + " words)";
      var s = strength(bits);
      rating.textContent = s.label;
      bar.style.width = Math.min(100, Math.round(bits / 128 * 100)) + "%";
      bar.style.background = s.color;
      show();
    }
    function show() { out.hidden = false; }
    words.addEventListener("input", function () { wordsVal.textContent = words.value; });
    go.addEventListener("click", gen);
    sep.addEventListener("change", gen);
