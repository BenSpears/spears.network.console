/* spears.network — self-contained Wi-Fi QR generator.
...
(function () {
  ...
  function wifiPayload(ssid, pass, auth, hidden) {
    ...
    if (auth === "nopass") return "WIFI:T:nopass;S:" + esc(ssid) + ";" + (hidden ? "H:true;" : "") + ";";
    return "WIFI:T:" + auth + ";S:" + esc(ssid) + ";P:" + esc(pass) + ";" + (hidden ? "H:true;" : "") + ";";
  }

  /* export for Node-based testing */
  if (typeof module !== "undefined" && module.exports) {
    module.exports = { encode: encode, wifiPayload: wifiPayload };
    return;
  }
  ...
  document.addEventListener("DOMContentLoaded", function () {
    var ssid = $("wf-ssid"), pass = $("wf-pass"), auth = $("wf-auth"),
        hidden = $("wf-hidden"), go = $("wf-go"), out = $("wf-out"),
        qr = $("wf-qr"), err = $("wf-err"), pngBtn = $("wf-png"), svgBtn = $("wf-svg");
    if (!go) return;
    var current = null;

    function fail(msg) { err.textContent = msg; err.hidden = false; out.hidden = true; }
    ...
    function render() {
      err.hidden = true;
      if (!ssid.value.trim()) return fail("Enter a network name (SSID).");
      if (auth.value !== "nopass" && !pass.value) return fail("Enter the password, or set security to None.");
      var text = wifiPayload(ssid.value.trim(), pass.value, auth.value, hidden.checked);
      var res;
      try { res = encode(text); }
      catch (e) {
        return fail(e.message === "too-long"
          ? "That SSID and password are too long to encode. Shorten them and try again."
          : "Couldn't generate the code.");
      }
      current = res;
      qr.innerHTML = svgFor(res.matrix, res.size, 4, 8);
      out.hidden = false;
    }
    ...
    function downloadSvg() {
      if (!current) return;
      var svg = svgFor(current.matrix, current.size, 4, 10);
      download("data:image/svg+xml;charset=utf-8," + encodeURIComponent(svg), "wifi-qr.svg");
    }
    function downloadPng() {
      ...
      download(cv.toDataURL("image/png"), "wifi-qr.png");
    }

    go.addEventListener("click", render);
    pass.addEventListener("keydown", function (e) { if (e.key === "Enter") render(); });
    ssid.addEventListener("keydown", function (e) { if (e.key === "Enter") render(); });
